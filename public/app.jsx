...
function normalizeDB(db) {
  ...
  if (!db.version || db.version < 4) db.version = 4;
  return db;
}
function loadDB() {
...
function seedDB() {
  const db = { users: [], conversations: [], members: [], messages: [], invites: [], nextIds: { user: 1, conversation: 1, message: 1, invite: 1 }, version: 4 };
        ...
        {convosFiltered.map(c => {
          ...
          return (
            <div key={c.id} className="flex items-center justify-between p-2 rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <div className="flex items-center gap-2">
                {c.type==='LOBBY' && <Button className="bg-gray-700" onClick={()=>onOpenInfo(c.id)}>Info</Button>}
                {c.type==='GROUP' && <Button className="bg-gray-700" onClick={()=>onOpenInfo(c.id)}>Info</Button>}
